...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<!--]--></div> `);
    {
      ...
      $$renderer2.push(`<!----> MANUAL PEER CONNECT</label> <input type="text"${attr("value", manualMultiaddr)} placeholder="Warden Multiaddr..." class="w-full bg-background border border-border p-3 text-sm focus:border-primary outline-none text-white font-mono"/></div> <button class="bg-secondary text-white px-6 py-3 font-bold hover:bg-white/10 transition-colors border border-border w-full md:w-auto"${attr("disabled", manualConnecting, true)}>${escape_html("DIRECT CONNECT")}</button></div> `);
      {
        $$renderer2.push("<!--[-->");
